...
const Filter = require('./src/models/Filters');
...
app.post('/:filterName', async (req, res) => {
  ...
  if (!["typeOptions", "locationOptions"].includes(filterName)) {
    return res.status(400).json({ error: "Invalid filter name" });
  }

  try {
    const filterDoc = await Filter.findOne();
    if (!filterDoc) return res.status(404).json({ error: "Filters not found" });

    if (!filterDoc[filterName].includes(option)) {
      filterDoc[filterName].push(option);
      await filterDoc.save();
    }

    res.json({ success: true, addedTo: filterName, option });
  } catch (error) {
