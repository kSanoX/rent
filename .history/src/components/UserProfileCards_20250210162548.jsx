...
const UserProfileCards = () => {
  ...
  const handleDelete = (cardId) => {
    ...
    fetch(`http://localhost:5000/api/user-cards/${cardId}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`,
      },
    ...
    .then(data => {
      if (data.success) {
        // Удаляем карточку из состояния после успешного удаления
        setUserCards(prevCards => prevCards.filter(card => card._id !== cardId));
      } else {
        setError(data.error || 'Ошибка при удалении карточки');
      }
    })
    .catch(err => {
      console.error("Ошибка при удалении:", err);
      setError(`Ошибка при удалении карточки: ${err.message}`);
    });
  };
