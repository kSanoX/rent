...
const FiltersForProperties = ({ onFiltersChange }) => {
  ...
  const getFilterOptions = (filter) => {
    ...
    return filter.options.map((option) => {
      // Если это объект с числовыми ключами и содержит поле _id, игнорируем его
      if (typeof option === "object" && option !== null && !Array.isArray(option)) {
        // Удаляем поле _id (если оно есть)
        const { _id, ...rest } = option;
        return Object.values(rest).join(""); // Объединяем все значения в строку
      }
      ...
      // Если это другой тип данных, возвращаем его как строку
