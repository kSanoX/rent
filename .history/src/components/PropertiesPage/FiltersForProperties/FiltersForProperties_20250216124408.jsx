...
const FiltersForProperties = ({ onFiltersChange }) => {
  ...
  const selectOption = (filterName, option) => {
    ...
    // Если это фильтр с диапазоном, то выводить выбранный диапазон
    if (filterName === "price" || filterName === "propertySize") {
      const selectedOption = option;
      const displayValue = `${selectedOption.min} - ${selectedOption.max}`;
      newFilterInputs[filterName] = displayValue;
    }
      ...
      {filterData.map((filter, index) => (
        ...
        >
          ...
          <input
            ...
            onChange={(e) => handleInputChange(filter.name, e.target.value)}  // Добавлено
            readOnly={activeFilter === filter.name ? false : true}  // Теперь инпут можно редактировать
          />
