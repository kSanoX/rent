...
const FiltersForProperties = ({ onFiltersChange }) => {
  ...
  const getFilterOptions = (filter) => {
    ...
    return filter.options.map((option) => {
      ...
      // Если это объект с индексами: {0: "K", 1: "y", 2: "i", 3: "v"}
      if (
        typeof option === "object" &&
        option !== null &&
        Object.keys(option).every((key) => !isNaN(key))
      ) {
        return Object.values(option).join(""); // Сливаем символы в строку
      }
      ...
      // Фолбэк на случай странного объекта (например, если это объект с вложенными данными)
