...
const FiltersForProperties = ({ onFiltersChange }) => {
  ...
  const getFilterOptions = (filter) => {
    ...
    return filter.options.map((option) => {
      // Обработка диапазона для цены и размера
      if (
        filter.name === "price" ||
      ...
      // Если это объект с индексами (например, {0: "K", 1: "y", 2: "i", 3: "v"})
      if (
        typeof option === "object" &&
        option !== null &&
        !Array.isArray(option) &&  // Убедимся, что это именно объект, а не массив
        Object.keys(option).every((key) => !isNaN(key)) // Проверка, что объект имеет числовые ключи
      ) {
        return Object.values(option).join(""); // Объединяем значения объекта в строку
      }
  
      // Фолбэк на случай других объектов (например, вложенные данные)
