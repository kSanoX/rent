...
const FiltersForProperties = ({ onFiltersChange }) => {
  ...
  const getFilterOptions = (filter) => {
    ...
    if (filter.name === "price" || filter.name === "propertySize") {
    ...
    // Если опция — это объект со строками по индексам (0, 1, 2), то превратим в массив
    if (filter.options.length === 1 && typeof filter.options[0] === "object") {
      const objectWithIndexes = filter.options[0];
      return Object.keys(objectWithIndexes)
        .filter(k => !isNaN(k)) // Только числовые ключи
        .map(k => objectWithIndexes[k]);
    }
