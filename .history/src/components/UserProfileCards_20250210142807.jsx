...
const UserProfileCards = ({editingApartment}, {openModal}) => {
  ...
  useEffect(() => {
    if (editingApartment) {
      setTitle(editingApartment.title);
      setDescription(editingApartment.description);
      setPrice(editingApartment.price);
      setBedroomCount(editingApartment.bedroomCount);
      setBathroomCount(editingApartment.bathroomCount);
      setType(editingApartment.type);
      setLocation(editingApartment.location);
      setBuildYear(editingApartment.buildYear);
      setPropertySize(editingApartment.sqm);
    } else {
      setTitle("");
      setDescription("");
      setPrice("");
        ...
        <ul className='profile-cards-list'>
          {userCards.map(card => (
            <li key={card._id} className='profile-cards-item'>
              ...
              <button onClick={() => openModal(apartment)}>Edit</button>
