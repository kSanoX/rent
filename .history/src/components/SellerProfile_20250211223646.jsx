...
function SellerProfile() {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:5000/api/users/${id}`, {
            ...
            .catch(err => {
                setError(`Ошибка при загрузке данных профиля: ${err.message}`);
                setIsLoading(false);
            });
        ...
        fetch('http://localhost:5000/api/user-cards', {
            ...
            .catch(err => {
                setError(`Ошибка при загрузке данных карточки: ${err.message}`);
            });
    ...
    if (!user) return <p>User not found or data is still loading...</p>; // Добавим проверку
