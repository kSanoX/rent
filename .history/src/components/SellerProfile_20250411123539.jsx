import { useEffect, useState } from "react";
import { useParams } from "react-router-dom";
import defaultAvatar from "../../src/images/profile-avatar-default.png";
import "../styles/sellerProfile.scss";
import ReviewModal from "./ReviwModal"; // consider using relative path
import { useAuth } from "../context/AuthContext";

function SellerProfile() {
    const { id } = useParams();
    const [user, setUser] = useState(null);
    const [cards, setCards] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const [isModalOpen, setIsModalOpen] = useState(false); // Modal state
    const { user: currentUser } = useAuth();

    useEffect(() => {
        if (!id) return;

        setIsLoading(true);

        fetch(`http://localhost:5000/api/users/${id}`)
            .then(response => {
                if (!response.ok) throw new Error(`Ошибка: ${response.status} ${response.statusText}`);
                return response.json();
            })
            .then(data => {
                setUser(data);
                setIsLoading(false);
            })
            .catch(err => {
                setError(`Error loading profile: ${err.message}`);
                setIsLoading(false);
            });

        fetch(`http://localhost:5000/api/user-cards/${id}`)
            .then(response => {
                if (!response.ok) throw new Error(`error: ${response.status} ${response.statusText}`);
                return response.json();
            })
            .then(data => {
                setCards(data.cards || []);
            })
            .catch(err => {
                setError(`error loading cards: ${err.message}`);
            });
    }, [id]);

    if (isLoading) return <p>Loading...</p>;
    if (error) return <p>{error}</p>;
    if (!user) return <p>User not found, and loading...</p>;

    return (
        <>
            <div className="seller-profile">
                <div className="profile-header">
                    <div className="profile-avatar">
                        <h1>{user.firstName} {user.lastName}</h1>
                        <p>{user.role}</p>
                        <img
                            src={user.avatar ? `http://localhost:5000${user.avatar}` : defaultAvatar}
                            alt={`${user.firstName} ${user.lastName}`}
                            className="profile-avatar"
                        />
                    </div>
                    {currentUser && currentUser.role === "buyer" && (
                        <div className="leave_rating" style={{ marginTop: "20px" }}>
                            <button
                                className="btn-reviwe"
                                style={{ color: "#E4E4E7" }}
                                onClick={() => setIsModalOpen(true)}
                            >
                                Leave a rating
                            </button>
                        </div>
                    )}
                </div>
                <div className="profile-details">
                    <div className="about">
                        <h1>About</h1>
                        <p>{user.message || "This seller hasn't provided a bio."}</p>
                    </div>
                    <div className="contact">
                        <h1>Contact</h1>
                        <p>Email: {user.email || "No instructions"}</p>
                        <p>Phone: {user.phone || "No instructions"}</p>
                    </div>
                </div>
                {cards.length > 0 ? (
                    <ul className='profile-cards-list_public'>
                        {cards.map(card => (
                            <li key={card._id} className='profile-cards-item_public'>
                                <h4>Title: {card.title}</h4>
                                <img src={`http://localhost:5000${card.image}`} className='profile-cards-img_public' alt="Card" />
                                <p>Price: {card.price}$</p>
                            </li>
                        ))}
                    </ul>
                ) : (
                    <p>No cards</p>
                )}
            </div>

            {/* review modal */}
            {isModalOpen && <ReviewModal userId={id} onClose={() => setIsModalOpen(false)} />}
        </>
    );
}

export default SellerProfile;
