...
const UserProfileCards = ({ editingApartment, setEditingApartment, openModal }) => {
  ...
  const handleEditClick = (apartment) => {
    setEditingApartment(apartment); // Устанавливаем выбранную квартиру для редактирования
        ...
        <ul className='profile-cards-list'>
          {userCards.map(card => (
            <li key={card._id} className='profile-cards-item'>
              ...
              <button onClick={() => navigate(`/propertyDetails/${card._id}`)}>Details</button>
              </div>
            </li>
          ))}
        </ul>
