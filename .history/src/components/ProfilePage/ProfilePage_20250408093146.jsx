...
const ProfilePage = () => {
  const { user, setUser, loading } = useAuth(); // используем loading из контекста
  ...
  useEffect(() => {
    ...
    const fetchLocations = async () => {
      try {
        ...
        if (data.success) {
          const locationFilter = data.location.find(
  ...
  if (loading) return <div>Загрузка...</div>; // ← покажи спиннер, пока грузится
  if (!user) return <Navigate to="/login" />;  // ← редирект если пользователь null
