...
const ProfilePage = () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          data.forEach(filter => {
            ...
            if (keyWithOptions) {
              optionsOnly[filter.name] = filter[keyWithOptions];
            }
          });
  ...
  const handleAddNewOption = (filterName, newOption) => {
    setFilters((prev) => {
      const updated = { ...prev };
      if (filterName === "type") {
        updated.typeOptions = [...prev.typeOptions, newOption];
      } else if (filterName === "location") {
        updated.locationOptions = [...prev.locationOptions, newOption];
      }
      return updated;
    });

    fetch(`http://localhost:5000/api/filters/${filterName}`, {
      method: "POST",
      headers: {
