      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          data.forEach(filter => {
            ...
            const keyWithOptions = Object.keys(filter).find(key => key.endsWith("Options"));
            if (keyWithOptions) {
              console.log("Key with options:", keyWithOptions); // Логирование ключа, который найден
              // Преобразуем массив символов в строку
              optionsOnly[filter.options] = filter[keyWithOptions].map(option => {
                return option.value.join(''); // Преобразуем массив в строку
              });
            }
          });
