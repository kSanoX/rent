...
const ProfilePage = () => {
  ...
  const [imageUrlFromCloudinary, setImageUrlFromCloudinary] = useState(null);
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      setImageUrlFromCloudinary(data.imageUrl);  // Сохраняем URL изображения
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/user/update', {
        ...
        body: JSON.stringify({ firstName, lastName, phone, message, avatarUrl: imageUrlFromCloudinary }),  // Отправляем данные из состояния
      });
