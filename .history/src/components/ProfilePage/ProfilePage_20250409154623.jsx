      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          data.forEach((filter) => {
            ...
            // Если есть опции у фильтра
            if (filter.options && Array.isArray(filter.options)) {
              const formattedOptions = filter.options.map((option) => {
                // Проверяем, что option это объект
                if (typeof option === "object" && option !== null && !Array.isArray(option)) {
                  const { _id, ...rest } = option; // Убираем _id и собираем остальные значения
                  return Object.values(rest).join(""); // Собираем строку из значений
                }
                // Если option это строка
                if (typeof option === "string") {
                  return option;
                }
                // Если что-то другое, сериализуем в строку
                return JSON.stringify(option);
              }).filter(value => value); // Фильтруем пустые значения
    
              optionsOnly[filter.name] = formattedOptions; // Добавляем отформатированные опции в объект
            } else {
              console.log("No options found for filter:", filter.name);
            }
          });
    
          setFilters(optionsOnly); // Устанавливаем полученные опции
          console.log("Only options:", optionsOnly); // Логируем отфильтрованные опции
        } else {
          console.error("Unexpected filters format:", data);
        }
      } catch (error) {
