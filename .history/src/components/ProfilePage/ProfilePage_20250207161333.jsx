...
const ProfilePage = () => {
  ...
  const handleAddApartment = async (e) => {
    ...
    formData.append("Pricing_Range", price);
    ...
    sliderImages.forEach((sliderImage, index) => {
      formData.append(`sliderImg[${index}]`, sliderImage);
    });
              ...
              {user?.role === "admin" && (
                  <div className="admin-panel">
                    ...
                    <form onSubmit={handleAddApartment}>
                      ...
                      <input type="text" placeholder="Локация" value={location} onChange={(e) => setLocation(e.target.value)} required />
                      <input type="number" placeholder="Год постройки" value={buildYear} onChange={(e) => setBuildYear(e.target.value)} required />
                      <input type="text" placeholder="Размер площади" value={propertySize} onChange={(e) => setPropertySize(e.target.value)} required />
                      <input type="file" onChange={(e) => setImage(e.target.files[0])} required />
                      <div>
                        <label>Изображения для слайдера (можно добавить несколько):</label>
                        <input type="file" multiple onChange={(e) => setSliderImages([...sliderImages, ...e.target.files])} />
                      </div>
