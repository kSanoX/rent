    ...
    const fetchFilters = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          data.forEach(filter => {
            console.log("Filter:", filter); // Логируем текущий фильтр
    
            // Если у фильтра есть опции и они в виде массива
            if (filter.options && Array.isArray(filter.options)) {
              const formattedOptions = filter.options.map(option => {
                console.log("Option:", option); // Логируем текущую опцию
    
                // Проверяем, что в опции есть значение, и собираем строку из значений
                if (option && option.value) {
                  return option.value.join(''); // Собираем строку, если value существует
                } else {
                  console.log("Empty or invalid option:", option);
                  return null; // Игнорируем пустые или некорректные значения
                }
              }).filter(value => value); // Убираем пустые значения из массива
    
              // Сохраняем отформатированные опции в объект
              optionsOnly[filter.name] = formattedOptions;
            } else {
              console.log("No options found for filter:", filter.name); // Если опции нет
            }
          });
    
          // Устанавливаем отфильтрованные опции в состояние
          setFilters(optionsOnly);
          console.log("Only options:", optionsOnly); // Логируем итоговые опции
        } else {
          console.error("Unexpected filters format:", data); // Если формат данных не соответствует ожиданиям
        }
      } catch (error) {
        console.error("Error fetching filters:", error); // Логируем ошибку при запросе
      }
    };
                    ...
                    <form onSubmit={handleAddApartment} className='apartment-form'>
                        ...
                        <AutocompleteInput
                          ...
                          onAddNew={async (newValue) => {
                            try {
                              const res = await fetch("http://localhost:5000/api/filters/add-option", {
                                method: "POST",
                                headers: {
                                  "Content-Type": "application/json",
                                  Authorization: `Bearer ${localStorage.getItem("token")}`,
                                },
                                body: JSON.stringify({ filterName: "type", option: newValue }),
                              });
                              ...
                              if (res.ok) {
                                ...
                                console.log("Option added:", newValue);
                              } else {
                          ...
                          onAddNew={async (newValue) => {
                            try {
                              const res = await fetch("http://localhost:5000/api/filters/add-option", {
                                ...
                                body: JSON.stringify({ filterName: "location", option: newValue }),
                              });

                              if (res.ok) {
                                setFilters((prev) => ({
                                  ...prev,
                                  location: [...(prev.location || []), newValue],
                                }));
                                console.log("Option added:", newValue);
                              } else {
                                console.error("Failed to add option");
