    ...
    const fetchFilters = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          data.forEach(filter => {
            console.log("Filter:", filter); // Логируем каждый фильтр
    
            // Если у фильтра есть опции
            if (filter.options && Array.isArray(filter.options)) {
              const formattedOptions = filter.options.map(option => {
                // Проверим структуру каждого option
                console.log("Option:", option);
    
                // Собираем строку из символов, если значение существует
                if (option && option.value) {
                  return option.value.join(''); // Собираем строку
                } else {
                  console.log("Empty or invalid option:", option);
                  return null; // Игнорируем пустые или некорректные значения
                }
              }).filter(value => value); // Фильтруем пустые строки
