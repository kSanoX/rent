...
const storage = multer.diskStorage({
  ...
  router.post("/", authMiddleware, verifyRole(["admin"]), upload.single("image"), async (req, res) => {
    try {
      const { title, description, price, bedroomCount, bathroomCount, type, propertySize, buildYear } = req.body;
      const imagePath = "/uploads/" + req.file.filename;

      // Создание нового объекта ApartmentCard с дополнительными полями
      const newApartment = new ApartmentCard({
        ...
        type,
        image: imagePath,
        propertySize,    // Добавление поля propertySize
        buildYear,       // Добавление поля buildYear
      });
